...
class BayesFilter():
    ...
    def _create_feature_extractor_params(self, args):
        ...
        for i in range(len(args.extractor_size)):
            ...
            self.extractor_w.append(tf.compat.v1.get_variable("extractor_w"+str(i), [prev_size, args.extractor_size[i]], 
                                                regularizer=tf.keras.regularizers.l2(0.5 * (args.reg_weight))))
            self.extractor_b.append(tf.compat.v1.get_variable("extractor_b"+str(i), [args.extractor_size[i]]))

        # Last set of weights to map to output
        self.extractor_w.append(tf.compat.v1.get_variable("extractor_w_end", [args.extractor_size[-1], args.code_dim], 
                                                regularizer=tf.keras.regularizers.l2(0.5 * (args.reg_weight))))
        self.extractor_b.append(tf.compat.v1.get_variable("extractor_b_end", [args.code_dim]))

    # Function to run inputs through extractor
    def _get_extractor_output(self, args, states):
        extractor_input = states
        for i in range(len(args.extractor_size)):
            extractor_input = tf.nn.relu(tf.compat.v1.nn.xw_plus_b(extractor_input, self.extractor_w[i], self.extractor_b[i]))
        output = tf.compat.v1.nn.xw_plus_b(extractor_input, self.extractor_w[-1], self.extractor_b[-1])
        return output
    ...
    def _gen_sample(self, args, dist_params):
        ...
        samples = tf.random.normal([args.batch_size, args.noise_dim])
        w = samples*w_std + w_mean
        w = tf.minimum(tf.maximum(w, -10.0), 10.0)
        w = tf.cond(pred=self.generative, true_fn=lambda: samples, false_fn=lambda: w)  # Just sample from prior for generative model
        return w

    # Bidirectional LSTM to generate initial sample of w1, then form z1 from w1
    def _create_initial_generator(self, args):
        fwd_cell = tf.compat.v1.nn.rnn_cell.LSTMCell(args.rnn_size, initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"))
        bwd_cell = tf.compat.v1.nn.rnn_cell.LSTMCell(args.rnn_size, initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"))
        
        # Get outputs from rnn and concatenate
        outputs, _ = tf.compat.v1.nn.bidirectional_dynamic_rnn(fwd_cell, bwd_cell, self.features[:, :args.seq_length], dtype=tf.float32)
        output_fw, output_bw = outputs
        output = tf.concat([output_fw[:, -1], output_bw[:, -1]], axis=1)

        # Single affine transformation into w1 distribution params
        hidden = tf.compat.v1.layers.dense(output, 
                                args.transform_size, 
                                activation=tf.nn.relu,
                                name='to_hidden_w1', 
                                kernel_regularizer=tf.keras.regularizers.l2(0.5 * (args.reg_weight)))
        self.w1_dist = tf.compat.v1.layers.dense(hidden, 
                                2*args.noise_dim, 
                                name='to_w1', 
                                kernel_regularizer=tf.keras.regularizers.l2(0.5 * (args.reg_weight)))
        ...
        hidden = tf.compat.v1.layers.dense(self.w1, 
                                args.transform_size, 
                                activation=tf.nn.relu,
                                name='to_hidden_z1', 
                                kernel_regularizer=tf.keras.regularizers.l2(0.5 * (args.reg_weight)))
        self.z1 = tf.compat.v1.layers.dense(hidden, 
                                args.code_dim, 
                                name='to_z1', 
                                kernel_regularizer=tf.keras.regularizers.l2(0.5 * (args.reg_weight)))
    ...
    def _create_transition_matrices(self, args):
        self.A_matrices = tf.compat.v1.get_variable("A_matrices", [args.num_matrices, args.code_dim, args.code_dim], 
                                            regularizer=tf.keras.regularizers.l2(0.5 * (args.reg_weight)))
        self.B_matrices = tf.compat.v1.get_variable("B_matrices", [args.num_matrices, args.action_dim, args.code_dim], 
                                            regularizer=tf.keras.regularizers.l2(0.5 * (args.reg_weight)))
        self.C_matrices = tf.compat.v1.get_variable("C_matrices", [args.num_matrices, args.noise_dim, args.code_dim], 
                                            regularizer=tf.keras.regularizers.l2(0.5 * (args.reg_weight)))
    ...
    def _create_weight_network_params(self, args):
        ...
        self.weight_w.append(tf.compat.v1.get_variable("weight_w1", [args.code_dim+args.action_dim, args.transform_size], 
                                            regularizer=tf.keras.regularizers.l2(0.5 * (args.reg_weight))))
        self.weight_w.append(tf.compat.v1.get_variable("weight_w2", [args.transform_size, args.num_matrices], 
                                            regularizer=tf.keras.regularizers.l2(0.5 * (args.reg_weight))))

        self.weight_b.append(tf.compat.v1.get_variable("weight_b1", [args.transform_size], 
                                            regularizer=tf.keras.regularizers.l2(0.5 * (args.reg_weight))))
        self.weight_b.append(tf.compat.v1.get_variable("weight_b2", [args.num_matrices], 
                                            regularizer=tf.keras.regularizers.l2(0.5 * (args.reg_weight))))
    ...
    def _create_inference_network_params(self, args):
        ...
        for i in range(len(args.inference_size)):
            ...
            self.inference_w.append(tf.compat.v1.get_variable("inference_w"+str(i), [prev_size, args.inference_size[i]], 
                                                regularizer=tf.keras.regularizers.l2(0.5 * (args.reg_weight))))
            self.inference_b.append(tf.compat.v1.get_variable("inference_b"+str(i), [args.inference_size[i]]))

        # Last set of weights to map to output
        self.inference_w.append(tf.compat.v1.get_variable("inference_w_end", [args.inference_size[-1], 2*args.noise_dim], 
                                                regularizer=tf.keras.regularizers.l2(0.5 * (args.reg_weight))))
        self.inference_b.append(tf.compat.v1.get_variable("inference_b_end", [2*args.noise_dim]))

    # Function to get weights for transition matrices
    def _get_weights(self, z, u):
        z_u = tf.concat([z, u], axis=1)
        hidden = tf.nn.relu(tf.compat.v1.nn.xw_plus_b(z_u, self.weight_w[0], self.weight_b[0]))
        return tf.nn.softmax(tf.compat.v1.nn.xw_plus_b(hidden, self.weight_w[1], self.weight_b[1]))
    ...
    def _get_inference_sample(self, args, features, z, u):
        ...
        for i in range(len(args.inference_size)):
            inference_input = tf.nn.relu(tf.compat.v1.nn.xw_plus_b(inference_input, self.inference_w[i], self.inference_b[i]))
        w_dist = tf.compat.v1.nn.xw_plus_b(inference_input, self.inference_w[-1], self.inference_b[-1])
    ...
    def _propagate_solution(self, args):
        ...
        for t in range(args.seq_length, 2*args.seq_length):
            ...
            # Draw noise sample and append sample to list
            w_t = tf.random.normal([args.batch_size, args.noise_dim])
    ...
    def _create_decoder_params(self, args):
        ...
        for i in range(len(args.extractor_size)-1, -1, -1):
            if i == len(args.extractor_size)-1:
                ...
                prev_size = args.extractor_size[i+1]
            self.decoder_w.append(tf.compat.v1.get_variable("decoder_w"+str(len(args.extractor_size)-i), [prev_size, args.extractor_size[i]], 
                                                regularizer=tf.keras.regularizers.l2(0.5 * (args.reg_weight))))
            self.decoder_b.append(tf.compat.v1.get_variable("decoder_b"+str(len(args.extractor_size)-i), [args.extractor_size[i]]))

        # Last set of weights to map to output
        self.decoder_w.append(tf.compat.v1.get_variable("decoder_w_end", [args.extractor_size[0], args.state_dim], 
                                                regularizer=tf.keras.regularizers.l2(0.5 * (args.reg_weight))))
        self.decoder_b.append(tf.compat.v1.get_variable("decoder_b_end", [args.state_dim]))

    # Function to run inputs through decoder
    def _get_decoder_output(self, args, encodings):
        decoder_input = encodings
        for i in range(len(args.extractor_size)):
            decoder_input = tf.nn.relu(tf.compat.v1.nn.xw_plus_b(decoder_input, self.decoder_w[i], self.decoder_b[i]))
        output = tf.compat.v1.nn.xw_plus_b(decoder_input, self.decoder_w[-1], self.decoder_b[-1])
        return output
    ...
    def _create_optimizer(self, args):
        ...
        self.loss_reconstruction = tf.reduce_sum(input_tensor=tf.square(self.x - self.rec_sol))
        ...
        w_dist = tf.compat.v1.distributions.Normal(loc=w_mean, scale=w_std)
        prior_dist = tf.compat.v1.distributions.Normal(loc=tf.zeros_like(w_mean), scale=tf.ones_like(w_std))
        self.kl_loss = tf.reduce_sum(input_tensor=tf.compat.v1.distributions.kl_divergence(w_dist, prior_dist))

        # Sum with regularization losses to form total cost
        self.cost = self.loss_reconstruction + self.kl_weight*self.kl_loss + tf.reduce_sum(input_tensor=tf.compat.v1.losses.get_regularization_losses()) 
        ...
        with tf.GradientTape() as tape:
            loss_value =  self.cost
        
        self.grads, _ = tf.clip_by_global_norm(tape.gradient(loss_value, tf.compat.v1.trainable_variables()), args.grad_clip)
        #self.grads = tape.gradient(loss_value, tvars)
